...
#     print(filename[j]+'x')
    ...
    AU, BU, CU = plt.hist(d_all, np.arange(-1.025,1.05,0.05), histtype='bar', facecolor='blue', alpha=0.75, rwidth=0.2)  # , density=True)
...
for i in range(len(filename)):  #  2,3):    #
    ...
    ax1.scatter(pdf_trans_dict[filename[i] + 'x'], pdf_trans_dict[filename[i] + 'y'], alpha=0.75,
