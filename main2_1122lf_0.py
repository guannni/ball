# lf == low frequency 球的面积改变较大的情况20hz,4g,2g
# # 主程序，获得球心位置，点位置，旋转矩阵
...
path1 = 'G:\\ball_new\\1_frame\\20hz_240\\1217\\' #  'G:\\guan_data\\2_ball\\1_pic\\60Hz\\'  # 读入路径
...
# ------圈球，标圆心
        ...
        circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=200, param2=15, minRadius=int(math.floor(circle3[0][0][2]-5)), maxRadius=int(math.ceil(circle3[0][0][2]+5)))  # TODO 把半徑範圍縮小點，檢測內圓，瞳孔
        if circle1 is None:
            circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=100, param2=15,minRadius=int(math.floor(circle3[0][0][2]-10)), maxRadius=int(math.ceil(circle3[0][0][2]+10)) ) # TODO 把半徑範圍縮小點，檢測內圓，瞳孔
            if circle1 is None:
                circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=150, param2=20, minRadius=59, maxRadius=75)
                if circle1 is None:
                     circle1 = circle3
                x,y,r = circle1[0][0][0],circle1[0][0][1],circle1[0][0][2]
            else:
...
# ------标记表面点的中心 (cv2.fitellipse)
        ...
        for cnt in cnts:
            if len(cnt)<5:
                ...
                ellipse = cv2.fitEllipse(cnt) # ((x,y),(a,b),theta) 中心坐标，半长轴半短轴，倾斜角
                if np.max(ellipse[1]) < 35 and np.min(ellipse[1])>r*0.1:
                    points.append(np.asarray(ellipse[0]))
                    cv2.ellipse(ball_o, ellipse, (255,0, 255), 1, cv2.LINE_AA)
                    cv2.circle(ball_o, (round(ellipse[0][0]),round(ellipse[0][1])), 2, (255, 0, 0),1)  # 畫椭圆圓心
                else:
