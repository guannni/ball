...
path2 = 'D:\\guan2019\\2_ball\\2_data_new\\60Hz_trans_acf_selected\\'  
...
# 用1/150s的时候用这个筛选
 ...
 # ACF________________________________________-
    ...
    if len(delta[:, 1]) > 5000:  # 10000:#
        dx = delta[:5000, 0]
        dy = delta[:5000, 1]
    v_t = (dx+dy)#/2.0 / 480 * 260
# # method 1 --my function-------
    ...
    ax.scatter(time[0:len(ac)], ac / ac[0], c='', alpha=0.65,s=5, edgecolor=colors[j],label=frame_name + 'g')  # auto correlation
    # ax.scatter(time[0:len(ac)], ac / ac[0], c=colors[j],marker='+', s=3,label=frame_name + 'g', cmap='hsv')  # auto correlation
    ax.plot(time[0:len(ac)], ac / ac[0], alpha=0.65,lw=1, color=colors[j],label=frame_name + 'g')  # auto correlation
