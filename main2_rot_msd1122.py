...
path4 = 'G:\\ball_new\\3_analysis\\msd\\rot_axisangle\\'  
# path4 = 'G:\\ball_new\\3_analysis\\msd\\rot_euler1\\' 
# path4 = 'G:\\ball_new\\3_analysis\\msd\\rot_qua4\\' #'D:\\guan2019\\2_ball\\2_data_new\\60Hz_rot(qua4)_msd\\' 

filename = [name for name in os.listdir(path2)]

pdf_rot_dict = {}

for j in range(len(filename)-1,len(filename)):  #3,4):    # 
...
# 求theta的MSD
        ...
        # traj = pd.DataFrame({'t': time, 'x': np.cumsum(deltaeuler[:, 0]), 'y': np.zeros(len(deltaeuler[:, 0]))})  # euler method -- theta
