...
path2 = 'G:\\ball_new\\2_data\\frame\\20hz_240_copy\\' # 'D:\\guan2019\\2_ball\\2_data\\60Hz_select_rot\\'  # TODO: 注意 这里只用_copy文件夹的数据！！！！
...
for j in range(len(filename)):  #3,4):    # len(filename)-1,
    ...
    # # translational-----------------------------------------------------------------------------------------
        ...
        # print(traj.head())
        # ax = traj.plot(x='x', y='y', alpha=1, legend=False, title='trajectory')
        # ax.set_xlim(0, 1000)  # (traj['x'].min()-10, traj['x'].max()+10)
        # ax.set_ylim(0, 1000)  # (traj['y'].min()-10, traj['y'].max()+10)
    ...
    # rotational--------------------------------------------------------------------------------------------------------------
        ...
        time = np.around(np.arange(0, (len(d_new_theta) ) * 1 / fps, 1 / fps), decimals=3) #np.arange(0, (len(d_new_theta) ) ))#
        d_new_theta = np.cumsum(d_new_theta)/time[:len(d_new_theta)]
        d_new_theta[np.isinf(d_new_theta)]=0
        ...
        # ax1 = plt.subplot(111)
        # time = np.around(np.arange(0, (len(d_new_theta) ) * 1 / fps, 1 / fps), decimals=3) #np.arange(0, (len(d_new_theta) ) ))#
        # plt.plot(time[:len(np.cumsum(d_new_theta[np.where(np.abs(d_new_theta)<30)]))],np.cumsum(d_new_theta[np.where(np.abs(d_new_theta)<30)])/time[:len(np.cumsum(d_new_theta[np.where(np.abs(d_new_theta)<30)]))], '.',markersize=1, alpha=0.4, label=filename1[i] + 'g')  # /180.0*math.pi   color=colors[i],
        # # ax1.set_yscale('log')
        # # ax1.set_xscale('log')
        # ax1.set_title(r' Angular Displacement' + ' [20Hz] ', fontsize=10)  # the Value of
    ...
    # # -----------
    #     # quaternions & 排列组合的fluctuation
    #     fig = plt.figure()
    #     ax1 = plt.subplot(111)
    #     time = np.around(np.arange(0, (len(quaternions[:,0])) * 1 / 240, 1 / 240), decimals=3) #np.arange(0, (len(d_new_theta) ) ))#
    #     plt.plot(time[:len(quaternions[:,0])],np.sqrt(quaternions[:,0]**2+quaternions[:,3]**2), '.',markersize=1, alpha=0.4, label=filename1[i])  # quaternions
    #     ax1.set_title(r' np.sqrt(quaternions0**2+quaternion1**2)' + ' [20Hz] ', fontsize=10)  # the Value of
    #     ax1.set_ylabel(r'$\Delta \Theta(°)$')
    #     ax1.set_xlabel(r'$time~(s)$')
    #     ax1.plot()
    #     plt.show()
    #     del fig, ax1
        ...
        # # ax = plt.subplot(111)
        # # plt.scatter(d_theta,d_new_theta,s=1)
        # ax = plt.subplot(111,projection='polar')
        # plt.scatter(np.cumsum(d_theta)%360,d_new_theta,s=1)
        # ax.set_ylim(0, 10)  # (traj['y'].min()-10, traj['y'].max()+10)
        
        # # ax.set_xlim(0, 1180)  # (traj['x'].min()-10, traj['x'].max()+10)
        # ax.set_xlabel(r'$\Delta \Theta(°)$')
        # ax.set_ylabel(r'$\Delta (\Delta \Theta)(°)$')
        # ax.plot()
...
# # euler----------------------------------------------------------------------------
#   # euler1
#     # d_euler1 = np.diff(d_euler[:,0])# -flow///////////////////////////
#     d_euler1 = d_euler[:,0]
...
#   # euler2
#     # d_euler2 = np.diff(d_euler[:,1])# -flow///////////////////////////
#     d_euler2 = d_euler[:,1]
...
#   # euler3
#     # d_euler3 = np.diff(d_euler[:,2]) # -flow///////////////////////////
#     d_euler3 = d_euler[:,2]
...
label = []
for i in range(len(filename)):  # 2,3):    #
    plt.scatter(pdf_rot_dict[filename[i] + 'x_new_theta'], pdf_rot_dict[filename[i] + 'p_new_theta'], alpha=0.5, edgecolor=colors[i],
                c='', s=25,cmap='hsv', label=filename[i])
    plt.plot(pdf_rot_dict[filename[i] + 'x_new_theta'], pdf_rot_dict[filename[i] + 'p_new_theta'], '-',alpha=0.4, color=colors[i],
                  label=filename[i] )  # /180.0*math.pi  color=colors[i],
...
# ax1.set_yscale('log')
# ax1.set_xscale('log')
ax1.set_title(r'PDF -- Angular Displacement' + ' [20Hz] ', fontsize=10)  # the Value of
...
# for i in range(len(filename)):  # 2,3):    #
#     # plt.scatter(pdf_rot_dict[filename[i] + 'x_theta'], pdf_rot_dict[filename[i] + 'p_theta'], alpha=0.5,
#     #             color=colors[i], cmap='hsv', label=filename[i] )
#     plt.plot(pdf_rot_dict[filename[i] + 'x_euler1'], pdf_rot_dict[filename[i] + 'p_euler1'], 'o-',alpha=0.5,
#                 color=colors[i],  label=filename[i] )
#     label.append(filename[i] )
# plt.legend(label)
# leg = ax1.legend(label)
# leg.get_frame().set_linewidth(0.0)
# ax1.set_title(r'PDF -- Euler $\alpha$' + ' [20Hz] ', fontsize=10)  # the Value of
...
# leg = ax1.legend(label)
# leg.get_frame().set_linewidth(0.0)
# ax1.set_title(r'PDF -- Euler $\beta $' + ' [20Hz] ', fontsize=10)  # the Value of
...
# leg = ax1.legend(label)
# leg.get_frame().set_linewidth(0.0)
# ax1.set_title(r'PDF -- Euler $\gamma$' + ' [20Hz] ', fontsize=10)  # the Value of
