# lf == low frequency 球的面积改变较大的情况20hz,6g
# # 主程序，获得球心位置，点位置，旋转矩阵
...
def points_ini(img):
    ...
    mask_ballindex = func_ball0913.create_circular_mask(ball_o.shape[0],ball_o.shape[1], center=[r, r], radius=r-2)  # TODO 加r-0.5的mask，遮挡球边阴影
...
path1 = 'G:\\ball_new\\1_frame\\20hz_240\\8g\\' #  'G:\\guan_data\\2_ball\\1_pic\\60Hz\\'  # 读入路径
filename = traversalDir_FirstDir(path1)
file_n = [path1 + name for name in filename]
print(file_n)

path2 = 'G:\\ball_new\\2_data\\frame\\20hz_240\\' # 'G:\\guan_data\\2_ball\\2_data\\1\\'  # 输出路径
...
# ------标记表面点的中心 (cv2.fitellipse)
        ...
        for cnt in cnts:
            if len(cnt)<5:
                ...
                ellipse = cv2.fitEllipse(cnt) # ((x,y),(a,b),theta) 中心坐标，半长轴半短轴，倾斜角
                if np.max(ellipse[1]) < 0.6*r and np.min(ellipse[1])>r*0.1:
                    points.append(np.asarray(ellipse[0]))
                    # cv2.ellipse(ball_o, ellipse, (255,0, 255), 1, cv2.LINE_AA)
                    # cv2.circle(ball_o, (round(ellipse[0][0]),round(ellipse[0][1])), 2, (255, 0, 0),1)  # 畫椭圆圓心
