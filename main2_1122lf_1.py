# lf == low frequency 球的面积改变较大的情况20hz6g,40hz2g
# # 主程序，获得球心位置，点位置，旋转矩阵
...
def points_ini(img):
    ...
    circle0 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 2000, param1=80, param2=30, minRadius=500, maxRadius=600)  # TODO 检测容器
    if len(circle0[0])==1:
    ...
    mask_contianer = func_ball0913.create_circular_mask(h,w, center=[w/2, h/2], radius=min(h,w)/2-2)  # TODO 加r-0.5的mask，遮挡球边阴影
    gray = 1.4*gray
    ...
    circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=50, param2=10, minRadius=54, maxRadius=120)  # TODO 把半徑範圍縮小點，檢測內圓，瞳孔
    if len(circle1[0])==1:
    ...
    for cnt in cnts:
        if len(cnt)<5:
            ...
            if np.max(ellipse[1]) < 0.6*r and np.min(ellipse[1])>0.15*r:
...
path1 = 'G:\\ball_new\\1_frame\\40hz_240\\40hz6g\\' #  'G:\\ball_new\\1_frame\\20hz_240\\6g\\'  # 读入路径
filename = traversalDir_FirstDir(path1)
file_n = [path1 + name for name in filename]
print(file_n)

path2 = 'G:\\ball_new\\2_data\\frame\\40hz_240\\' # 'G:\\ball_new\\2_data\\frame\\20hz_240\\'  # 输出路径
...
# ------圈球，标圆心
        ...
        circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=300, param2=15, minRadius=int(math.floor(circle3[0][0][2]-r/12)), maxRadius=int(math.ceil(circle3[0][0][2]+r/12)))  # TODO 把半徑範圍縮小點，檢測內圓，瞳孔
        if circle1 is None:
            circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=200, param2=15,minRadius=int(math.floor(circle3[0][0][2]-r/10)), maxRadius=int(math.ceil(circle3[0][0][2]+r/10)) ) # TODO 把半徑範圍縮小點，檢測內圓，瞳孔
            if circle1 is None:
                circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=150, param2=15, minRadius=59, maxRadius=75)
                if circle1 is None:
                    circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=150, param2=15, minRadius=75, maxRadius=115)
                    if circle1 is None:
                        circle1 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,1000, param1=150, param2=15, minRadius=115, maxRadius=155)
                        circle1 = circle3
                        x,y,r = circle1[0][0][0],circle1[0][0][1],circle1[0][0][2]
                    else:
                        x,y,r = circle1[0][0][0],circle1[0][0][1],circle1[0][0][2]
...
# ------提取球表面点
        ...
        for m in range(h):
            for n in range(w):
                ...
                distance = math.pow((h/2.0-n), 2) + math.pow((h/2.0-m), 2)
                if (distance < (h/2.0) **2):
                    result = (int)(strength*( 1.0 - math.sqrt(distance) / (h/2.0+30) ))#TODO 30 按照距离大小计算增强的光照值
                    f = max(0, ball_amp[n,m] + result)#判断边界 防止越界
                    highlightfilter[n,m] = np.uint8(f)
                else:
                    highlightfilter[n,m] = np.uint8(ball_amp[n,m])
...
# ------标记表面点的中心 (cv2.fitellipse)
        ...
        for cnt in cnts:
            if len(cnt)<5:
                ...
                ellipse = cv2.fitEllipse(cnt) # ((x,y),(a,b),theta) 中心坐标，半长轴半短轴，倾斜角
                if np.max(ellipse[1]) < 0.6*r and np.min(ellipse[1])>r*0.15:
                    points.append(np.asarray(ellipse[0]))
                    # cv2.ellipse(ball_o, ellipse, (255,0, 255), 1, cv2.LINE_AA)
                    # cv2.circle(ball_o, (round(ellipse[0][0]),round(ellipse[0][1])), 2, (255, 0, 0),1)  # 畫椭圆圓心
                elif np.max(ellipse[1]) < 0.7*r and np.min(ellipse[1])>r*0.15:
                    points.append(np.asarray(ellipse[0]))
        # print(points) 
        # # # 画图
        # fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(8, 5))
